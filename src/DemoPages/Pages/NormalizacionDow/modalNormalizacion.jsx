...
const ModalNormalizacion = (props) => {
  ...
  const [seleccionFlujoPermeado, setSeleccionFlujoPermeado] = useState([]);
  const [seleccionSales, setSeleccionSales] = useState([]);
  const [seleccionDiffPresion, setSeleccionDiffPresion] = useState([]);
  ...
  const getDatosNormSeleccion = () => {
    const formData = new FormData();
    formData.append("id_planta", props.planta.id);
    fetch(apis.getDatosNormSeleccion, {
      method: "POST",
      headers: {
      ...
      .then((json) => {
        const datosSelecFlujoPermeado = [];
        const datosSelecSales = [];
        const datosSelecDiffPresion = [];
        json.forEach((dato) => {
          switch (Number(dato.idPadre)) {
            case 22:
              datosSelecFlujoPermeado.push(dato);
              break;
            case 23:
              datosSelecSales.push(dato);
              break;
          }
        });
        setSeleccionFlujoPermeado(datosSelecFlujoPermeado);
        setSeleccionSales(datosSelecSales);
        setSeleccionDiffPresion(datosSelecDiffPresion);
      })
      .catch(() => {});
  };
  ...
  const datosNormalizacion = useMemo(() => {
    if (props.data.length <= 0) return [];

    return props.data.filter((items) => items !== null);
  }, [props.data]);

  useEffect(() => {
    getDatosNormSeleccion();
  }, []);
  ...
  const insertDatoNormSeleccion = (listaDatosSeleccionados) => {
    const formData = new FormData();
    const fechaUltimoRegistroDow = new Date(props.ultimoDato.fecha);
    formData.append("id_planta", props.planta.id);
    formData.append(
      "fecha_inicio",
      fechaUltimoRegistroDow.toLocaleDateString("en-US")
    );
    formData.append(
      "datos_seleccionados",
      JSON.stringify(listaDatosSeleccionados)
    );
    fetch(apis.insertDatoNormSeleccion, {
      method: "POST",
      headers: {
        "Id-User": user.id_user,
        "Token": user.token
  ...
  const onClickConfirmarCambioParametros = () => {
    const listaNuevosSeleccionados = [];
    /* insertDatoNormSeleccion(props.planta.id, fechaUltimoRegistroDow.toLocaleDateString("en-US"), 22, 17, selectFlujoPermTCF.id, selectFlujoPermTCF.tcf); */
    if (selectFlujoPermTCF.id != "") {
      listaNuevosSeleccionados.push({
        id_padre: 22,
        id_hijo: 17,
        id_dato_norm: selectFlujoPermTCF.id,
        valor: selectFlujoPermTCF.tcf
      });
    }
    if (selectFlujoPermNDP.id != "") {
      listaNuevosSeleccionados.push({
        id_padre: 22,
        id_hijo: 21,
        id_dato_norm: selectFlujoPermNDP.id,
        valor: selectFlujoPermNDP.ndp
      });
    }
    if (selectSalesFlujoPerm.id != "") {
      listaNuevosSeleccionados.push({
        id_padre: 23,
        id_hijo: 1,
        id_dato_norm: selectSalesFlujoPerm.id,
        valor: selectSalesFlujoPerm.flujo_permeado
      });
    }
    if (selectSalesTDSCalcAli.id != "") {
      listaNuevosSeleccionados.push({
        id_padre: 23,
        id_hijo: 6,
        id_dato_norm: selectSalesTDSCalcAli.id,
        valor: selectSalesTDSCalcAli.tds_calc_alimen
      });
    }
    if (selectSalesCalcFeedBrane.id != "") {
      listaNuevosSeleccionados.push({
        id_padre: 23,
        id_hijo: 18,
        id_dato_norm: selectSalesCalcFeedBrane.id,
        valor: selectSalesCalcFeedBrane.calc_feed_brine_avg
      });
    }

    insertDatoNormSeleccion(listaNuevosSeleccionados);
  };
    ...
    <Modal
      ...
      <div className="modal-padding">
        ...
        >
          ...
          <Grid
            ...
            <Grid
              ...
              spacing={3}>
              <Grid item>
                {!cambiarParametros && [5].includes(Number(user.perfil)) && (
                  <Button
                    color="info"
          ...
          <Grid
            ...
            <Grid
              ...
              <Grid
                ...
                <Grid item>
                  <button
                    onClick={onClickConfirmarCambioParametros}
        ...
        <ReactTable
          ...
          pageSize={datosNormalizacion.length}
          minRows={1}
          // pageSize={31}
