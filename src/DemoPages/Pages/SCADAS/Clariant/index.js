...
import ScadaClariant from "./ScadaClariant";
...
const Clariant = ({ idPlanta = 60 }) => {
  ...
  const [calculosDiarios, setCalculosDiarios] = useState({
    "id": -1,
    "id_planta": 0,
    "fecha": "",
    "volumen_alimentacion": 0,
    "volumen_mezcla": 0,
    "volumen_permeado": 0,
    "vol_alimen_mensual": 0,
    "vol_mezcla_mensual": 0,
    "vol_permeado_mensual": 0
  });
  // Estandarizable
  ...
  const getInfoGeneralPlanta = (signal) => {
    fetch(`${apis.getInfoGeneralPlanta}?id_planta=${idPlanta}`, {
      method: "GET",
      headers: {
        "Id-User": user.id_user,
        "Token": user.token
      ...
      .then((json) => {
        if (json) {
          if (json.calculos_diarios.length > 0)
            setCalculosDiarios(json.calculos_diarios[0]);
        }
      })
  ...
  const excelAbortController = useAbortController();

  const exportarDatosScada = () => {
    const controller = excelAbortController();
    const { signal } = controller;
    setLoadingExport(true);
    const fileName = `Clariant-${formatearFechaDDMMYY(startDate)}-${formatearFechaDDMMYY(endDate)}.xls`;
  ...
  // Estandarizable
  const [loadingTotalizadores, setLoadingTotalizadores] = useState(false);
  const totalizadoresAbortController = useAbortController();

  const exportarTotalizadores = () => {
    const controller = totalizadoresAbortController();
    const { signal } = controller;
    setLoadingTotalizadores(true);
    const fileName = `Clariant_totalizadores-${formatearFechaDDMMYY(startDate)}-${formatearFechaDDMMYY(endDate)}.xls`;
    const uri = `${apis.exportarTotalizadoresCariant}?id_planta=${idPlanta}&&fechainicio=${startDate.toLocaleDateString("en-US")}&&fechafin=${endDate.toLocaleDateString("en-US")}`;
    fetch(uri, {
      method: "GET",
      headers: {
        "Id-User": user.id_user,
      ...
      .finally(() => setLoadingTotalizadores(false));
  };
  ...
  const loadGraficoParametros = (
    ...
    fetch(apis.obtainGraphWithMult, {
      ...
      .catch((ex) => setLoadingDatosVariable(false))
      .finally(() => setLoadingDatosVariable(false));
  };
  ...
  // vibot on reset
  const { estadoRoD1, estadoRoD2, isVibotReset } = useVibotOnReset(datosScada);
    ...
    <>
      ...
      <div>
        <Breadcrumb className="Breadcrumb">
          ...
          <BreadcrumbItem>Clariant</BreadcrumbItem>
      ...
      >
        ...
        <Tarjetita
          number={12}
          name="Conductividad Alimentación"
        ...
        <Tarjetita
          number={14}
          name="Estado Bomba antiincrustante"
      ...
      <Grid
        ...
        <Tarjetita
          number={21}
          name="Volumen Alimentación Diario"
          value={calculosDiarios.volumen_alimentacion}
          unit="m³"
          clickMe={() => {}}
          size="x-large-tarjeta">
          {" "}
        </Tarjetita>
        <Tarjetita
          number={22}
          name="Volumen Producto Diario"
          value={calculosDiarios.volumen_permeado}
          unit="m³"
          clickMe={() => {}}
          size="x-large-tarjeta">
          {" "}
        ...
        <Tarjetita
          number={23}
          name="Volumen Rechazo Diario"
          value={(
            calculosDiarios.volumen_alimentacion -
            calculosDiarios.volumen_permeado
          ).toFixed(2)}
          unit="m³"
          clickMe={() => {}}
          size="x-large-tarjeta"
        ...
        <Tarjetita
          number={24}
          name="Volumen Alimentación Mensual"
          value={calculosDiarios.vol_alimen_mensual}
          unit="m³"
          clickMe={() => {}}
          size="x-large-tarjeta">
          {" "}
        ...
        <Tarjetita
          number={25}
          name="Volumen Producto Mensual"
          value={calculosDiarios.vol_permeado_mensual}
          unit="m³"
          clickMe={() => {}}
          size="x-large-tarjeta">
          {" "}
        ...
        <Tarjetita
          number={26}
          name="Volumen Rechazo Mensual"
          value={(
            calculosDiarios.vol_alimen_mensual -
            calculosDiarios.vol_permeado_mensual
          ).toFixed(2)}
          unit="m³"
          clickMe={() => {}}
          size="x-large-tarjeta">
          ...
          </Grid>
          {
            <Grid item>
              <div className="titles">
                <Button
                  onClick={() => {
                    if (loadingTotalizadores) return;
                    exportarTotalizadores();
                  }}
                  style={{
                    marginBottom: "4px",
                    cursor: loadingTotalizadores ? "not-allowed" : "pointer"
                  }}
                  color="info">
                  {loadingTotalizadores ? (
                    <div
                      style={{ paddingInline: "33px", paddingBlock: "4.8px" }}>
                      <Loader />
                    </div>
                  ) : (
                    "Totalizadores"
