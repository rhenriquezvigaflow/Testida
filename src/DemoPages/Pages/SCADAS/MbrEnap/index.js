...
import { getPlantasEstados } from "../../../../commons/services/getPlantasEstados";
import ModalRangoGraficos from "../ModalRangoGraficos.jsx";
import { ScadaMbrEnap } from "./ScadaMbrEnap.js";

export function MbrEnap({ idPlanta = 120 }) {
  ...
  const [isScadaReady, setIsScadaReady] = useState(false);
  ...
  const exportarDatosScada = () => {
    ...
    const fileName = `MBR-Enap-${formatearFechaDDMMYY(startDate)}-${formatearFechaDDMMYY(endDate)}.xls`;
  ...
  const onClickBuscarDatos = () => {
    ...
    const res = loadFuncionamientoPlanta(17);
  ...
  const [estadoMbr, setEstadoMbr] = useState(null);
  const [loadingEstado, setLoadingEstado] = useState(false);
  const [errorEstado, setErrorEstado] = useState(null);

  useEffect(() => {
    const obtenerEstado = async () => {
      if (!datosScada.d17 || !isScadaReady) return;

      setLoadingEstado(true);
      setErrorEstado(null);

      try {
        const data = await getPlantasEstados({
          idPlanta,
          idEstado: datosScada.d17
        });

        if (data) {
          setEstadoMbr(data.estado);
        }
      } catch (err) {
    ...
    obtenerEstado();
  }, [idPlanta, datosScada.d17, isScadaReady]);
  ...
  useEffect(() => {
    ...
    loadScada(signal);
    const res = loadFuncionamientoPlanta(17, signal);
    res.then((json) => setDatosFuncionamientoPlanta(json));
    getAlarm(signal);
  ...
  const { isVibotReset } = useVibotOnReset(datosScada);
    ...
    <>
      ...
      <div>
        <Breadcrumb className="Breadcrumb">
          ...
          <BreadcrumbItem>MBR Enap</BreadcrumbItem>
      ...
      <Grid
        ...
        <Tarjetita
          name="Estado MBR"
          number={17}
          value={estadoMbr ? estadoMbr.estado : null}
        ...
        />
        <Tarjetita
          name="Sensor de Oxigeno Estanque Biológico"
          number={2}
          value={datosScada.d2}
          selected={Number(selected)}
          max={umbrales.ma2}
