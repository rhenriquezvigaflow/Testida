...
import { getEPO, getScaledValue } from "../../../../commons/scada/scada";
import ModalRangoGraficos from "../ModalRangoGraficos.jsx";
import ScadaAquaChileRH from "./ScadaAquaChileRH";
...
const AquaChileRH = () => {
  ...
  const [datosGraficoMonitoreoContinuos, setDatosGraficoMonitoreoContinuos] =
    useState([]);
  const [startDate, setStartDate] = useState(sumarDias(new Date(), -7));
  ...
  const exportarDatosScada = () => {
    ...
    const fileName = `AquaChileRioHollemberg-${formatearFechaDDMMYY(startDate)}-${formatearFechaDDMMYY(endDate)}.xls`;
  ...
  return (
    <>
      ...
      <h1
        ...
        Planta Aqua Chile Río Hollemberg ORVF 8188
      </h1>
      <div>
        <Breadcrumb className="Breadcrumb">
          ...
          <BreadcrumbItem active>SCADA ORVF-8188</BreadcrumbItem>
        </Breadcrumb>
      </div>
      <Grid
        container
        ...
        <Tarjetita
          ...
          value={getScaledValue(datosScada.d8, 1)}
          selected={Number(selected)}
          max={umbrales.ma8}
          min={umbrales.mi8}
          unit="m³/h"
      ...
      <Grid
        ...
        <Tarjetita
          ...
          value={
            datosScada.d7 > 0
              ? ((datosScada.d8 / datosScada.d7) * 100).toFixed(1)
              : 0
          }
          selected={Number(selected)}
          max={umbrales.mi20}
